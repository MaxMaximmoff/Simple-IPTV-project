...
public class Parser {
    ...
    public List<Entry> parse(InputStream stream) {
        ...
        try {
            ...
            String prvdrName = extract(line, PROVIDER_NAME_REGEX);

            Entry.Builder entry = null;
            while ((line = buffer.readLine()) != null) {
                lineNbr++;
                if (isExtInfo(line)) {
                    entry = extractExtInfo(globalTvgShif, prvdrName, line);
                } else {
                    if (entry == null) {
    ...
    private Entry.Builder extractExtInfo(String globalTvgShift, String prvdrName, String line) {
        String duration = extract(line, DURATION_REGEX);
        String tvchId = extract(line, TVCH_ID_REGEX);
